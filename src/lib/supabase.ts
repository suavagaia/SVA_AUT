...
// Web: sessionStorage → a sessão sobrevive a F5 na mesma aba, mas é limpa ao
// fechar a aba/janela → exige novo login a cada nova sessão do navegador.
// App nativo (iOS/Android): localStorage → mantém o login entre aberturas do
// app, sem forçar login toda vez que o usuário abre o aplicativo.
const authStorage = Capacitor.isNativePlatform() ? window.localStorage : window.sessionStorage;
