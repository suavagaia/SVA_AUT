...
export default function ThankYouMonthly() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="mx-auto max-w-lg text-center">
        ...
        <h1 className="font-display text-3xl text-foreground">Assinatura Ativada!</h1>
        <p className="mt-2 text-muted-foreground">Bem-vindo ao Plano Mensal do Sua Vaga IA</p>

        <div className="mt-8 grid grid-cols-2 gap-4">
          {[
            { title: 'Acesso Ilimitado', desc: 'Todos os agentes' },
            { title: 'IA Avançada', desc: 'Tecnologia avançada' },
            { title: 'Suporte Prioritário', desc: 'Atendimento especial' },
            { title: 'Histórico Completo', desc: 'Conversas salvas' },
          ].map((item) => (
        ...
        <div className="mt-6 rounded-lg bg-amber-50 p-4 text-sm text-amber-800">
          🏆 Sua assinatura mensal está ativa e será renovada automaticamente por R$129,00/mês
        </div>
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Escolha seu concurso e comece sua jornada rumo à aprovação
        </p>
      </div>
    </div>
  );
